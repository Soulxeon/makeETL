...
def retrieveAllData(mode = 'a'):
    ...
    try:
        ...
        # execute a statement
        if mode == 'pending':
            cur.execute(f"""SELECT * FROM {settings.name_project} WHERE status = 'Pending';""")
        else:
            cur.execute(f"""SELECT * FROM {settings.name_project};""")
        ...
        if mode == 'pending':
            print("Data retrieved: " + str(len(df)) + " pending Files in DB")
